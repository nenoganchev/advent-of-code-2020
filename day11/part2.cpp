    ...
    for (int y = 0; y < map.rows(); ++y) {
        for (int x = 0; x < map.cols(); ++x) {
            ...
            case free_seat:
                ...
                break;
            case taken_seat:
                if (map.count_of_adjacent(taken_seat, x, y) >= 5) {
...
int waiting_area_map::count_of_adjacent(tile_type type, int x, int y) const {
    ...
    auto inside_map = [this](int x, int y) { return 0 <= x && x < this->x_end && 0 <= y && y < this->y_end; };
    ...
    for (auto [dx, dy] : neighbour_offsets) {
        ...
        while (inside_map(neighbour_x, neighbour_y) && this->at(neighbour_x, neighbour_y) == floor) {
            neighbour_x += dx;
            neighbour_y += dy;
        }

        if (inside_map(neighbour_x, neighbour_y) && this->at(neighbour_x, neighbour_y) == type) {
            ++count;
        }
    }
